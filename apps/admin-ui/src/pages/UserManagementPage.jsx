...
const UserManagementPage = () => {
  ...
  const openEditModal = (user) => {
      setIsEditing(true);
      setCurrentUser({ ...user, mappedProjects: user.mappedProjects || [] });
      setIsModalOpen(true);
  };
  
  const handleFormSubmit = async (e) => {
    e.preventDefault();
    isEditing ? await handleEditUser() : await handleAddUser();
  };

  const handleAddUser = async () => {
      if (!currentUser.email || !currentUser.password) return alert("Email and password are required.");
      try {
      ...
      } catch (error) { console.error(error); alert("Failed to add user document."); }
  };
  ...
  const handleDeleteUser = async (userId) => {
      if(window.confirm("Are you sure you want to delete this user record?")) {
          try { await deleteDoc(doc(db, "users", userId)); alert("User record deleted."); }
          catch(error) { console.error(error); alert("Failed to delete user document."); }
      }
  }
